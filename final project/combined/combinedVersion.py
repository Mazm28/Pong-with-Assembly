...
lib = ctypes.CDLL('./libcalculations.so')

# Define function prototypes for assembly functions
calculate_wave_offset_asm = lib.calculate_wave_offset
calculate_wave_offset_asm.argtypes = [ctypes.c_float]
calculate_wave_offset_asm.restype = ctypes.c_float

apply_gravity_asm = lib.apply_gravity
apply_gravity_asm.argtypes = [ctypes.POINTER(ctypes.c_float)]
apply_gravity_asm.restype = None

update_ball_x_asm = lib.update_ball_x
update_ball_x_asm.argtypes = [ctypes.c_float, ctypes.c_float]
update_ball_x_asm.restype = ctypes.c_float

update_ball_y_asm = lib.update_ball_y
update_ball_y_asm.argtypes = [ctypes.c_float, ctypes.c_float]
update_ball_y_asm.restype = ctypes.c_float

# Replace Python functions with assembly versions
def calculate_wave_offset(x: float) -> float:
    return calculate_wave_offset_asm(x)

def apply_gravity(ball_velocity_y: float) -> float:
    y = ctypes.c_float(ball_velocity_y)
    apply_gravity_asm(ctypes.byref(y))
    return y.value

def update_ball_x(ball_x: float, ball_velocity_x: float) -> float:
    return update_ball_x_asm(ball_x, ball_velocity_x)

def update_ball_y(ball_y: float, ball_velocity_y: float) -> float:
    return update_ball_y_asm(ball_y, ball_velocity_y)
...
# Create the AI goaler (used in AI mode only).
...
# game_mode options: "normal", "wave", "volleyball", "ai", "journey", and "records"
game_mode = None      
...
# ---------------- Target Variables for Non-AI Modes ----------------
...
journey_modes = ["normal", "wave", "volleyball"]
journey_step_index = 0       # 0 = normal, 1 = wave, 2 = volleyball
...
menu_options = ["Normal Mode", "Wave Mode", "Volleyball Mode", "AI Mode", "Journey", "Records"]
...
while running:
    ...
    # ----- Target Activation for Non-AI Modes (ignored in Journey mode) -----
    if game_mode not in ["ai", "journey"] and score >= 10 and not target_active:
        target_active = True
    ...
    # ----- Loss Condition: Ball crosses x = 0 (for all non-AI modes) -----
    if game_mode not in ["ai"] and ball.left <= 0:
        game_over = True
    ...
    if game_mode == "journey":
        ...
        elif current_underlying == "volleyball":
            ball.x = int(update_ball_x(ball.x, ball_velocity[0]))
            ball.y = int(update_ball_y(ball.y, ball_velocity[1]))
            ball_velocity[1] = apply_gravity(ball_velocity[1])
    elif game_mode in ("normal", "ai"):
        ball.x = int(update_ball_x(ball.x, ball_velocity[0]))
        ball.y = int(update_ball_y(ball.y, ball_velocity[1]))
    elif game_mode == "wave":
        ball.x = int(update_ball_x(ball.x, ball_velocity[0]))
    ...
    if ball.colliderect(paddle):
        ...
        if game_mode == "journey":
            ...
            if game_mode != "ai" and not target_active:
                score += 1
            elif game_mode == "ai":
                score += 1
    
    # ----- Right-Side Collisions -----
    if game_mode in ("normal", "wave", "volleyball") or game_mode == "journey":
    ...
    if game_mode not in ["ai", "journey"] and target_active and target_rect:
